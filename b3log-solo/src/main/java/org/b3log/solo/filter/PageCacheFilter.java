...
//        httpServletResponse.setHeader("Expires", "Mon, 25 Aug 1986 00:00:00 GMT");
        ...
        if (!Latkes.isPageCacheEnabled()) {
            doFilterForGeneration(chain, request, response,
                                  startTimeMillis);
        ...
        if (null == cachedPageContentObject) {
            ...
            try {
                ...
                if (Article.ARTICLE.equals(cachedType)) {
                    ...
                    LOGGER.log(Level.FINEST, "Oid[{0}]", oId);
    ...
    /**
     * Do filter with the specified filter chain, servlet request, servlet 
     * response and process start time.
     * 
     * @param chain the specified filter chain
     * @param request the specified servlet request
     * @param response the specified servlet response
     * @param startTimeMillis the specified process start time
     * @throws IOException io exception
     * @throws ServletException servlet exception
     */
    private void doFilterForGeneration(final FilterChain chain,
        ...
        final PrintWriter writer =
                getResponseWriter((HttpServletResponse) response);
        final String msg = String.format(
                "<!-- Generated by B3log Solo(%1$d ms), %2$s -->",
                endimeMillis - startTimeMillis, dateString);
        LOGGER.finer(msg);
        writer.append(msg); // FIXME: why this does not work?
        writer.flush();
        writer.close();
    }
