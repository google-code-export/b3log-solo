...
public final class CommentService extends AbstractGAEJSONRpcService {
	 ...
	 * Comment repository.
	 */
	private CommentRepository commentRepository = CommentGAERepository.getInstance();
	/**
	 * Event manager.
	 */
	 ...
	 * Article repository.
	 */
	private ArticleRepository articleRepository = ArticleGAERepository.getInstance();
	/**
	 ...
	 */
	private PageCommentRepository pageCommentRepository = PageCommentGAERepository.getInstance();
	/**
	 * Page repository.
	 */
	...
	public JSONObject getCommentsOfArticle(final JSONObject requestJSONObject,
		...
		try {
			...
			for (int i = 0; i < articleCommentRelations.size(); i++) {
				final JSONObject articleCommentRelation = articleCommentRelations.get(i);
				final String commentId = articleCommentRelation.getString(Comment.COMMENT + "_"
						+ Keys.OBJECT_ID);

				final JSONObject comment = commentRepository.get(commentId);
				final String content = comment.getString(Comment.COMMENT_CONTENT).replaceAll(
						SoloServletListener.ENTER_ESC, "<br/>");
	...
	public JSONObject getCommentsOfPage(final JSONObject requestJSONObject,
		...
		try {
			...
			final List<JSONObject> pageCommentRelations = pageCommentRepository.getByPageId(pageId);
			// Step 2: Get comments
			final List<JSONObject> comments = new ArrayList<JSONObject>();
			for (int i = 0; i < pageCommentRelations.size(); i++) {
				final JSONObject pageCommentRelation = pageCommentRelations.get(i);
				final String commentId = pageCommentRelation.getString(Comment.COMMENT + "_"
						+ Keys.OBJECT_ID);
	...
	public JSONObject removeCommentOfArticle(final JSONObject requestJSONObject,
		...
		try {
			...
			final String articleId = articleCommentRelation.getString(Article.ARTICLE + "_"
					+ Keys.OBJECT_ID);

			if (!userUtils.canAccessArticle(articleId)) {
				ret.put(Keys.STATUS_CODE, StatusCodes.REMOVE_COMMENT_FAIL_FORBIDDEN);
			...
			eventManager.fireEventSynchronously(new Event<String>(EventTypes.REMOVE_COMMENT,
					articleId));
	...
	public JSONObject removeCommentOfPage(final JSONObject requestJSONObject,
		...
		try {
			...
			final JSONObject pageCommentRelation = pageCommentRepository.getByCommentId(commentId);
			final String pageCommentRelationId = pageCommentRelation.getString(Keys.OBJECT_ID);
			pageCommentRepository.remove(pageCommentRelationId);

			final String pageId = pageCommentRelation.getString(Page.PAGE + "_" + Keys.OBJECT_ID);
			// Step 2: Remove comment
			commentRepository.remove(commentId);
			...
			// Step 5: Fire remove comment event
			eventManager
					.fireEventSynchronously(new Event<String>(EventTypes.REMOVE_COMMENT, pageId));
			...
			LOGGER.log(Level.FINER, "Removed comment[oId={0}] of page[{oId={1}}]", new String[] {
					commentId, pageId });
		} catch (final Exception e) {
			if (transaction.isActive()) {
				transaction.rollback();
