...
(function ($) {
    ...
    $.extend(Tabs.prototype, {
        ...
        _init: function (target) {
            ...
            for (var i = 0, j = 0; i < $tabs.length; i++) {
                if ($tabs[i].children[0].id) {
                    settings.data[j] = $tabs[i].children[0].id;
                    $tabs[i].children[0].onclick = function () {
                         $.tabs._setCurrentTab(this, target);
                    }
                    j++;
                }
            }
        ...
        _setCurrentTab: function (it, target) {
            var inst = this._getInst(target);
            var settings = inst.settings;
            var data = settings.data;
            for (var i = 0; i < data.length; i++) {
                var $panel = $("#" + data[i].replace("tab_", "tabs_")),
                $tab = $("#" + data[i]);
                if (it.id === data[i]) {
                    $panel.show();
        ...
        _addTabs: function (target, addData) {
            ...
            settings.data.push("tab_" + addData.id);
           var $src = $.tabs._analyse(id, addData.path);
            
            $src.after("<li><div id='tab_" + addData.id + "'>\
                            <a href='#" + addData.id + "'>" + addData.text + "</a>\
                        </div></li>");
            $("#tab_" + addData.id).click(function () {
                         $.tabs._setCurrentTab(this, target);
                    });
            $("#" + id + "Panel").append("<div id=" + id + "_" + addData.id + ">" + addData.content + "</div>");
        }, 
        
        _analyse: function (id, path) {
            var paths = path.split("/"),
            $target = undefined,
            firstPath = 0;
            
            $target = $("#tab_" + paths[2]).parent();
            return $target;
        }
    });
