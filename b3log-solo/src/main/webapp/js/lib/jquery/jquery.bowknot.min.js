...
(function ($) {
    ...
    $.extend(Tip.prototype, {
        ...
        _destroyTip: function (target) {
            $.removeData(target, PROP_NAME);
            $("#" + target.id + "Panel").remove();
        },
        ...
        _init: function (target) {
            ...
            var tipHTML = '',
            styleClass = settings.className ? settings.className :
            this._getDefaults($.tip._defaults, settings, "styleClass").panel;

            tipHTML = "<div id='" + id
            + "Panel' class='" + styleClass + "'>" + settings.content + "</div>";

            $(settings.appendId ? "#" + settings.appendId : "body").append(tipHTML);

           

            $(target).hover(function () { $.tip._parsePosition(settings, target);
                $("#" + id + "Panel").fadeIn("normal");
            }, function () {
                $("#" + id + "Panel").fadeOut("normal");
            });
        },

        _optionTip: function (target, optionName, value) {
            var inst = $.tip._getInst(target[0]);
            var settings = inst.settings;
            if (value) {
                settings[optionName] = value;
                $("#" + target[0].id + "Panel").html(value);
            }
            return settings[optionName];
        },

        _parsePosition: function (settings, target) {
            var positionHTML = "",
            $targetPanel = $("#" + target.id + "Panel");
            if (settings.left === undefined && settings.right === undefined
                && settings.bottom === undefined && settings.top === undefined
            ...
            if (settings.position) {
                ...
                switch (settings.position) {
                    case "top":
                        positionHTML = "left: " + parseInt($target.position().left) + "px; top: "
                        + parseInt($target.position().top - $targetPanel.height() - 6
                            - parseInt($target.css("padding-top")) - parseInt($targetPanel.css("padding-top"))) + "px;";
                        break;
                    case "bottom":
                        positionHTML = "left: " + parseInt($target.position().left) + "px; top: "
                        + parseInt($target.position().top + $target.height()) + "px;";
                        break;
                    default:
                        alert("Has no type!");
                        break;
                }
                $targetPanel.attr("style", positionHTML);
                return;
            }

            if (settings.left !== undefined) {
                positionHTML += "left:" + settings.left + "px;";
            }
            if (settings.top !== undefined) {
                positionHTML += "top:" + settings.top + "px;";
            }
            if (settings.bottom !== undefined) {
                positionHTML += "bottom:" + settings.bottom + "px;";
            }
            if (settings.right !== undefined) {
                positionHTML += "right:" + settings.right + "px;";
            }
            $targetPanel.attr("style", positionHTML);
        },
    ...
    $.fn.tip = function (options) {
        ...
        if (options === "option") {
            otherArgs.shift();
            return $.tip['_' + options + 'Tip'].apply($.tip, [this].concat(otherArgs));
        }
        return this.each(function () {
            typeof options == 'string' ?
            $.tip['_' + options + 'Tip'].
            apply($.tip, [this].concat(otherArgs)) :
            $.tip._attach(this, options);
        });
    };
