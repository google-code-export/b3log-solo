...
(function($){$.fn.extend({tabs:{version:"0.0.1.7",author:"lly219@gmail.com"}});var g=new Date().getTime();var h='tabs';var k=function(){};$.extend(k.prototype,{_attach:function(a,b){if(!a.id){this.uuid++;a.id='dp'+this.uuid}var c=this._newInst($(a));c.settings=$.extend({length:0},b||{});$.data(a,h,c);this._init(a)},_newInst:function(a){var b=a[0].id.replace(/([^A-Za-z0-9_])/g,'\\\\$1');return{id:b}},_getInst:function(a){try{return $.data(a,h)}catch(err){throw'Missing instance data for this tabs';}},_init:function(a){var b=this._getInst(a);var c=b.settings,id=b.id;var d=$(a).find("li");c.data=[];for(var i=0,j=0;i<d.length;i++){if(d[i].children[0].id){c.data[j]=d[i].children[0].id.replace(id+"_","");d[i].children[0].onclick=function(){$.tabs._setCurrentTab(this,a)};j++}}},_setCurrentTab:function(a,b){var c=this._getInst(b);var d=c.settings,id=c.id;var e=d.data;for(var i=0;i<e.length;i++){var f=$("#"+id+"Panel_"+e[i]),$tab=$("#"+id+"_"+e[i]);if(a.id===id+"_"+e[i]){f.show();$tab.find("a").addClass("tab-current")}else{f.hide();$tab.find("a").removeClass("tab-current")}}},_addTabs:function(a,b){var c=this._getInst(a);var d=c.settings,id=c.id;d.data.push(b.id);var e=$.tabs._analyse(id,b.path);e.after("<li><div id='"+id+"_"+b.id+"'>                            <a href='#"+b.id+"'>"+b.text+"</a>                        </div></li>");$("#"+id+"_"+b.id).click(function(){$.tabs._setCurrentTab(this,a)});$("#"+id+"Panel").append("<div id="+id+"Panel_"+b.id+">"+b.content+"</div>")},_removeTabs:function(a,b){var c=this._getInst(a);var d=c.settings.data,id=c.id;for(var i=0;i<d.length;i++){if(d[i]===b){d.splice(i,1)}}$("#"+id+"_"+b).remove();$("#"+id+"Pabel_"+b).remove()},_analyse:function(a,b){var c=b.split("/"),$target=undefined,firstPath=0;$target=$("#"+a+"_"+c[2]).parent();return $target}});$.fn.tabs=function(a){var b=Array.prototype.slice.call(arguments);if(typeof a==='string'){b.shift();return $.tabs['_'+a+'Tabs'].apply($.tabs,[this[0]].concat(b))}return this.each(function(){$.tabs._attach(this,a)})};$.tabs=new k();window['DP_jQuery_'+g]=$})(jQuery);