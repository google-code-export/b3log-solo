...
(function ($) {
    ...
    $.extend(Bowknot.prototype, {
        ...
        getDefaults: function (defaults, settings, key) {
            ...
            } else if (key === "height" && settings[key] !== "auto") {
...
(function ($) {
    ...
    $.extend(Paginate.prototype, {
        ...
        _build: function (target) {
            ...
            switch ($.bowknot.getDefaults($.paginate._defaults, settings, "style")) {
                case "taobao":
                    pages = this._getTaobaoPages(currentPage, pageCount);
                    break;
                case "google":
                    pages = this._getGooglePages(currentPage, pageCount, $.bowknot.getDefaults($.paginate._defaults, settings, "windowSize"));
                    break;
                default:
                    alert("has no style!");
                    break;
...
(function ($) {
    ...
    $.extend(Table.prototype, {
        ...
        _build: function (target) {
            ...
            if ($("#" + inst.id + "TableMain").attr("scrollHeight")
                ...
                if (tableWidth.width === "100%") {
                    ...
                    $("#" + inst.id + "TableMain").width(tableWidth.width + SCROLLBAR_WIDTH);
                }
            }
                ...
                for (var tempIndex in elements) {
                    ...
                    for (var index in elements) {
                        if(colModelIndex === index && colModel[j].visible === undefined) {
                            ...
                            if (colModel[j].style) {
                                dataElement = "<span style='" + colModel[j].style + "'>" + dataElement + "</span>";
                            }
        ...
        _mouseupVerticalLine: function (event) {
            ...
            var colModel = settings.colModel,
            indexWidth = $.table._removePX(verticalLineLeft),
            i = 0,
            tableWidth = 0;
        ...
        _selectHeaderCheckbox: function (it, highlightClass) {
            ...
            for (var i = 0; i < colModel.length; i ++) {
                if (colModel[i].index === index) {
                    for (var j = 0; j < data.length; j++) {
                        if (it.checked) {
                            data[j][index] = true;
                            $("#" + id + "_" + index + "_" + j).attr("checked","checked");
                            temp.push(j);
                        } else {
                            data[j][index] = false;
                            $("#" + id + "_" + index + "_" + j).removeAttr("checked");
                        }
                    }
                }
            }
            ...
            if (it.checked) {
                ...
                $("#" + id + "TableMain tr").addClass(highlightClass);
            } else {
                for (var element in $.table._getInvisibleData(settings.colModel, data, [0])) {
                    $(it).removeData(element);
                }
        ...
        _removePX: function (px) {
            return parseInt(px.substring(0, px.length - 2));
        },

        _getMarginPadding: function (id) {
            var marginLeft = $.table._removePX(document.getElementById(id).style.marginLeft),
            paddingLeft = $.table._removePX(document.getElementById(id).style.paddingLeft);
        ...
        _getPadding: function (id) {
            var paddingLeft = $.table._removePX(document.getElementById(id).style.paddingLeft);
            if (document.getElementById(id).style.paddingLeft === "") {
                paddingLeft = 0;
            }
            return paddingLeft;
    ...
    $.extend(Dialog.prototype, {
        ...
        _getInst: function (target) {
            try {
                ...
                throw 'Missing instance data for this bowknot';
            }
        },
        ...
        _init: function (target) {
            ...
            $("#" + id + "Dialog").show(200).css({
                height: height,
                width:  width,
                top: (document.body.clientHeight - $("#" + id + "Dialog").height()) / 2,
                left: (document.body.clientWidth - $("#" + id + "Dialog").width()) / 2
            });
        },
...
(function ($) {
    ...
    $.extend(Loading.prototype, {
        ...
        _getInst: function (target) {
            try {
                ...
                throw 'Missing instance data for this loading';
            }
        },
        ...
        _init: function (target) {
            ...
            styleHTML = "";
            if (settings.width) {
                styleHTML += "height:" + settings.heignt + "px;";
            }
            if (settings.height) {
                styleHTML += "width:" + settings.width + "px;";
            }
            $('#' + id + "Span").remove();
            var loadHTML = "<span id='" + id + "Span'><img style='" + styleHTML + "' src='" + settings.imgURL+ "' id='" + id + "Img'></span>"
            $('#' + id ).append(loadHTML);
        },

        _closeLoading: function (target) {
            $('#' + target.id + "Img").remove();
        }
    });
    ...
    $.fn.loading = function (options) {
        ...
        return this.each(function () {
            typeof options == 'string' ?
            $.loading['_' + options + 'Loading'].
            apply($.loading, [this].concat(otherArgs)) :
            $.loading._attach(this, options);
        });
    };
